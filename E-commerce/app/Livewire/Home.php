...
class Home extends Component
{
    ...
    $featuredProducts = Product::whereBetween('regular_price', [$this->min_value, $this->max_value])
        ->orderBy('updated_at', 'desc') // Change this to your desired field for Featured tab
        ->take($this->selectedOption)
        ->get();

    $popularProducts = Product::whereBetween('regular_price', [$this->min_value, $this->max_value])
        ->orderBy('updated_at', 'desc') // Change this to your desired field for Popular tab
        ->take($this->selectedOption)
        ->get();

    $newAddedProducts = Product::whereBetween('regular_price', [$this->min_value, $this->max_value])
        ->orderBy('created_at', 'desc') // Change this to your desired field for New Added tab
        ->take($this->selectedOption)
        ->get();
